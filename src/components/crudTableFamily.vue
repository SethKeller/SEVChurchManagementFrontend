...
import FamilyServices from "../services/FamilyMemberServices";
    export default {
        ...
        computed: {
            formTitle() {
                return this.editedIndex === -1 ? 'New Family' : 'Edit Item';
            },
        },
        methods: {
             goToFamily(family) {
      // Go to the family details page
      this.$router.push("/family-info/" + family.id);
    },
            getFamilys()
            {
            FamilyServices.getFamilies().then((response) => {
